...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    div.innerHTML = `<div class="single-product">
      <div onclick="details(${product.id})">
    <img class="product-image" src=${image}></img>
      </div>
      <div class="description-bar">
      <h3>${product.title}</h3>
      <p>Category: ${product.category}</p>
      <p><i class="fas fa-star icon-star"></i> ${product.rating.rate}(${product.rating.count})</p>
      <h2>Price: $ ${product.price}</h2>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-success">add to cart</button>
      <button type="button"  id="details-btn" data-bs-toggle="modal" data-bs-target="#exampleModal${product.id}" class="btn btn-danger">Details</button>
      </div>
      </div>
...
// details button load data 
const details=async(detail)=>{
  const url=`https://fakestoreapi.com/products/${detail}`
...
const displayDetails=(data)=>{
  ...
  moda.textContent=""
  const div=document.createElement("div")
  div.classList.add("extra_detail")
  div.innerHTML=`
  <h4>${data.title}</h4>
